...
public class OllamaClient {
  ...
  public async Task<List<string>> GenerateRankingOrderAsync(string rankingPrompt, string model = "deepseek-r1:8b", int? max_tokens = null) {
    var requestBody = new {
      model,
      messages = new[] {
            new {
                role = "system",
                content = "You are an AI assistant specialized in ranking search results based on their relevance to a given query.\n" +
                          "Your task is to order the provided search results from most relevant to least relevant.\n" +
                          "Strictly follow the JSON schema and return only a ranked list of document IDs."
            },
            new {
                role = "user",
      ...
      response_format = new {
        ...
        json_schema = new {
          ...
          schema = new {
            type = "object",
            properties = new Dictionary<string, object> {
                        { "ranked_ids", new { type = "array", items = new { type = "string" } } }
                    },
            required = new[] { "ranked_ids" },
            additionalProperties = false
          },
          strict = true
        }
    ...
    return responseData?.choices?.FirstOrDefault()?.message?.content != null
        ? JsonSerializer.Deserialize<Dictionary<string, List<string>>>(responseData.choices[0].message.content)?["ranked_ids"] ?? []
        : [];
  }
